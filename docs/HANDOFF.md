...
- At handoff time HEAD is `494a578` ("Flag Claude-authored pages for human
  review"), working tree clean, branch pushed and in sync with origin.
...
links — preserve those exactly. Replace only the `## TODO:` sections, and strip
the `TODO:` prefix from the heading text (keep the rest of the heading).
...
2. **Verify writes with the Read tool, not just shell echo.** A `Write` was
   silently corrupted once this session (10k garbage lines). After writing,
   check line count and the last line; if a file looks wrong, rewrite it.
...
4. **Mark authored pages.** Add to the frontmatter of every page you author:
   ```yaml
   author: Claude
   review: Needs Human Review
   ```
   These extra keys are ignored by `validate_metadata` and the template, so they
   don't affect the build or tests. Then add the page to the "Needs Human
   Review" list in `TODO.md`. Only tag pages you *author* — for pages you merely
   correct, note them in TODO without the `author` flag.
5. **Style guardrails** (lint each file before commit):
...
```
`.venv` and shared assets are already fetched. If shared assets are missing,
run `make fetch` (needs network). Quick lint for a file:
```sh
f=content/donor-guides/02-wall-chargers.md
grep -c TODO "$f"; grep -c '—' "$f"; wc -l "$f"; tail -1 "$f"
```
...
- **Milestone 1 (Foundations) complete:** `01-why-salvage`, `02-safety`,
  `03-tools-and-workspace`, `04-core-techniques` — all authored + review-flagged.
- **Milestone 2 (Donor guides): 1 of 10 done** — `01-battery-devices` authored
  + flagged.

**Remaining donor-guide stubs (author next, low hazard → high):**
- [ ] `02-wall-chargers` (hazard 2)
- [ ] `03-routers-modems` (hazard 2)
- [ ] `04-printers` (hazard 2)
- [ ] `05-desktop-computers` (hazard 2)
...
order listed, using `write-donor-guide`. For each: preserve the existing table
and theory links, replace the TODO sections, add the `author`/`review`
frontmatter, lint, `make build` + pytest, then commit and push. Update the
"Needs Human Review" list in `TODO.md` as you go. Pay particular attention to
the hazard-4 guides (microwave, CRT, UPS): every stored-energy or live-source
hazard must be paired with a discharge or handling procedure, consistent with
`content/foundations/02-safety.md`.

Start by reading `docs/ROADMAP.md`, `docs/STYLE-GUIDE.md`, and an exemplar
(`07-atx-power-supplies.md`), then begin with `02-wall-chargers.md`.
