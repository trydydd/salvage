...
D-PAK (also called TO-252) is the SMD equivalent: a flat rectangular body with a large exposed pad on the bottom that's soldered to the board for heat dissipation, plus three leads on one side. You'll find D-PAK MOSFETs in laptop chargers, phone chargers, and anything that needed to manage power in a compact space. SO-8 packages carry dual MOSFETs on a single eight-pin chip, and these appear in motor H-bridges and battery protection boards.

The part number tells you the channel type, though the conventions aren't universal. International Rectifier's IRF-prefix parts are almost always N-channel unless the numeric code starts with 9 (IRF9540 is P-channel, IRF540 is N-channel) or the suffix ends in "P." FQP30N06L breaks down as N-channel (the N), nominally 30A but rated 32A continuous on the datasheet, 60V. IRFP9240 is P-channel (die codes starting with 9 in the IRF family are P-channel). If the marking is worn, the body diode test below will tell you which channel you have.
...
Set your meter to diode mode. On an N-channel MOSFET, the body diode runs from source to drain with the anode at the source. To find it: probe each combination of the three pins until one pair reads a forward voltage of 0.45–0.75V with the red probe on one pin and the black probe on the other. That forward-biased reading is the body diode. The pin with the red probe is the source, the pin with the black probe is the drain. The remaining pin is the gate.
...
To do a basic switching test at the bench, connect a resistor (470Ω to 1kΩ) in series with the drain to a supply voltage (3–5V works for the drain side). Connect source to ground. With the gate floating or shorted to source, the MOSFET should be off and no current flows through the resistor. Apply a separate gate drive voltage of 5–10V gate-to-source: the MOSFET turns on and you can measure voltage across the resistor with the meter. This confirms the gate oxide is intact and the device switches.
...
The most common failure in power MOSFETs. Overvoltage on the drain (from an inductive kick, a mains transient, or a motor back-EMF spike) exceeds the drain-source breakdown voltage and triggers destructive avalanche in the drain-body depletion region, leaving it conducting in both directions. In diode mode, you'll read a low voltage (often 0.05–0.3V, lower than a normal body diode) in both drain-source directions. The tab is often discolored or the package cracked. Don't reuse it.
