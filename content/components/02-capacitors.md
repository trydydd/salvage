...
Tantalum capacitors are the small teardrop-shaped or rectangular parts, usually brown, yellow, or a faded orange. The positive lead is marked with a stripe or a "+" symbol. SMD tantalums look like a squat rectangular block and often have a painted stripe on the positive end, not the negative end. That's the opposite of electrolytics, where the stripe marks negative. It's the most common polarity mistake with this family. They run small values (0.1µF–100µF), more stable in capacitance over time than standard electrolytics, and they show up in portable electronics on logic supply rails. They fail short rather than open. The failure mode has real consequences for reuse, so read the Failure modes section before you stock them. Supercapacitors look like large electrolytics but carry values in farads, not microfarads. They're worth pulling, but a 1F/5V supercap holds around 12J of energy, far more than a standard electrolytic at the same voltage. Discharge through a resistor before handling, same as a large electrolytic.
...
Before testing any capacitor pulled from a high-voltage circuit (camera flash units, CRT monitors, microwave ovens, or the primary side of a mains switching supply), measure the voltage across the leads with your meter before touching them. Capacitors in those circuits can hold a dangerous charge for hours after power is removed. A photoflash cap at 300V stores around 15J. A microwave HV cap at 2100V can be lethal. Discharge through a 10kΩ resistor rated for the voltage, held across the leads until the reading drops to millivolts.

**Multimeter resistance mode (quick check)**

Desolder the capacitor first. In-circuit checks for capacitors are largely useless because parallel components swamp the reading. Set your meter to a high resistance range (2MΩ or the highest available). Touch the probes to the capacitor leads, observing polarity on electrolytics (red probe to positive). The reading should start low, then climb toward OL as the capacitor charges from the meter's internal battery. Reverse the probes and watch it discharge and charge again. A capacitor that reads a dead short (zero, staying at zero) is failed short. One that reads OL immediately with no charge sweep is open or dry. That test is only informative on electrolytics, though. Small ceramics and film caps charge too fast at a meter's test current to show any sweep. They'll read OL almost immediately whether they're good or bad, so test those in capacitance mode instead.

**Multimeter capacitance mode**

If your meter has a capacitance range, discharge the capacitor first. For small ceramics and film caps, shorting the leads for a few seconds is fine. For electrolytics above about 100µF, or anything pulled from a high-voltage circuit, use a resistor instead: hold a 100Ω to 1kΩ resistor rated for the voltage across the leads until the reading drops to millivolts. Then connect to the meter. For electrolytics, observe polarity. The reading should be within 20% of the marked value for a healthy part. Readings more than 50% low on an electrolytic usually indicate significant degradation; anything below 20% of the marked value is worth discarding. C0G/NP0 and X7R ceramics should read within 10–20% of marked. Y5V and Z5U types have rated tolerances of ±30% at room temperature and can swing much wider with temperature, so a reading outside that window doesn't mean the part is bad. Film caps should read within 10–20% of marked.
...
Tantalum capacitors fail short, often catastrophically. Reverse polarity or overvoltage is the usual cause, but tantalums can also fail short if the rated voltage is exceeded even briefly. A shorted tantalum in a 5V logic rail will draw continuous current, heat up, and sometimes catch fire. Solid tantalum caps use manganese dioxide as the electrolyte, not liquid. When the dielectric shorts, the resulting current can trigger a self-heating reaction in the tantalum metal and oxide that sustains itself without any external ignition. Before reusing any tantalum, check it in resistance mode with the probes in both orientations: a good part charges like an electrolytic; a shorted part reads zero in both directions.
...
Ceramic and film capacitors from healthy boards are almost always worth pulling. They don't degrade with age the way electrolytics do, and a ceramic marked 104 is the same part regardless of when it was made. Film caps with a safety class marking (X1, X2, Y1, Y2) or a VDE/UL logo are a different case. Those are mains-rated safety components and can only go back into a role that maintains the same safety class. Don't use an X2 cap as a general-purpose film cap. It was placed there for a specific mains-filtering job and the safety classification matters. Prioritize larger physical sizes (1206 and above for SMD, standard disc sizes for through-hole) because they're easier to work with. The tiny 0402 SMD parts are not worth the effort unless you specifically need that package.
